...
<table cellpadding="8" cellspacing="0" style="margin: 16px 0; width: 100%;">
    ...
    <tr><td style="font-size: 14px; color: #888;">Total:</td><td style="font-size: 14px; font-weight: bold; color: #28a745;">${{ number_format($order->total, 2) }}</td></tr>
    <tr><td style="font-size: 14px; color: #888;">Items:</td><td style="font-size: 14px;">{{ $order->items->count() }} item(s)</td></tr>
</table>
@foreach($order->items as $item)
<p style="font-size: 13px; margin: 2px 0;">• {{ $item->product_name }} × {{ $item->quantity }} = ${{ number_format($item->total, 2) }}</p>
@endforeach
@endsection
